...
# SPEED VARS
turnSpeedDefault=1.6
maxstraightSpeed=2.3
minstraightSpeed=0.04
backwardsMaxSpeed=1
holdCurrent=11 # between 1 and 32
...
def write_gconf():
    write_reg(0,GCONF,((multistep_filt*256 + 512*test_mode)+(I_scale_analog + 2*internal_Rsense + 4*en_spreadcycle + 8*shaft + 16*index_otpw + 32*index_step + 64*pdn_disable + 128*mstep_reg_select)))

def write_current():
    write_reg(0,IHOLD_IRUN,(65536+(holdCurrent-1)+(runCurrent-1)*256))
...
uart2 = UART(1, baudrate=115200,  bits=8, parity=None, stop=1, tx=Pin(8), rx=Pin(9)) 
...
try:
    ...
    while button.value() == 0:
        pass
    startTime = ticks_ms() + startTimeOffset*(1000) # motor time offset, in nanoseconds
    ...
    print("")
    printlcd(f'Time: {(ticks_ms() - (startTime-startTimeOffset*1000))/1000:.2f}s')
    # BUZZ (for fun)
    buzzer.freq(750)
    buzzer.duty_u16(1000)
    led.off()
