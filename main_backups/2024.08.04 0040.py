...
def s(cm, ending):
    ...
    if ending == False:
        ...
        for i in range3:
            ...
            step_pin.value(0)
            sleep_us(delay[i*-1])
        #print(f"Elapsed Time: { (ticks_ms() - starttime2) / 1000 } seconds")
    elif ending == True:
        range2=range(iAtEnd-1, (2+steps-iAtEnd)-3600)
        range3=range(1, 3600)
        range4=range((-1*iAtEnd)+2, 0)
        starttime2=ticks_ms()
        for i in range(1, iAtEnd-1):
            step_pin.value(1)
            step_pin.value(0)
...
def turn(degrees, speedLimit):
    ...
    del delay2
    command_number += 1  # Shift position to next command
    if round(degrees) == 90 or degrees == -90:
    ...
    for index in range(pos, len(arr)):
        ...
        if i.startswith("s"):
            ...
            if cm > 0 or straightSpeed < backwardsMaxSpeed:
                ...
                timeLeft += cm/straightSpeed + saccel_delay
            else:
                # extra time for acceleration
                timeLeft += cm/straightSpeed + saccel_delay
        elif i.startswith("t"):
            timeLeft += (float(i.strip("t"))/90)*turnTime
